...
func (this *Space_UserController) Save() {
	...
	privilege := strings.TrimSpace(this.GetString("privilege", ""))
	...
	if err != nil {
		this.ErrorLog("空间导入节点失败: " + err.Error())
...
func (this *Space_UserController) Remove() {
	...
	err := models.SpaceUserModel.DeleteBySpaceIdAndUserId(spaceId, userId)
	if err != nil {
...
func (this *Space_UserController) Modify() {
	...
	privilege := this.GetString("privilege", "")
	userId := this.GetString("user_id", "")
	spaceId := this.GetString("space_id", "")
